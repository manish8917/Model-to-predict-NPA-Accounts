...
The Bank has not done well in last few quarters. Their NPAs (Non Performing Assets) have reached all time high. It is starting to lose confidence of its investors. As a result, it’s stock has fallen by 20% in the previous quarter alone. After careful analysis, it was found that the majority of NPA was contributed by loan defaulters. With the messy data collected over all the years, this bank has decided to use machine learning to figure out a way to find these defaulters and devise a plan to reduce them. This bank uses a pool of investors to sanction their loans. 
For example: If any customer has applied for a loan of $20000, along with bank, the investors perform a due diligence on the requested loan application. Keep this in mind while understanding data. In this challenge, we will help this bank by predicting the probability that a member will default.

#### Architecture (Flow)

![image](https://user-images.githubusercontent.com/40944253/192782083-85e39378-9fcb-4c3d-9b87-faeb4a9f1051.png)

    
#### Data Description
    
The client will send data in multiple sets of files in batches at a given location. Data will contain LoanIDs and 30 columns of Loan Accounts values for each account. The last column will have the "Good/Bad" value for each wafer.

"Good/Bad" column will have two unique values 0 and 1.  
"0" represents Bad LoanID.
"1" represents Good LoanID. 
